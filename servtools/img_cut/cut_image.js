...
CutImageClass.prototype = {
	...
	begin_cut_image: function(){
		...
		var cutDivPostionx=$(this.image_id).offsetWidth/2-(this.width/2);
		var cutDivPostiony=$(this.image_id).offsetHeight/2-(this.height/2);		
		
		var image_x = tmpPosition[0]+cutDivPostionx;
		var image_y = tmpPosition[1]+cutDivPostiony;
	...
	end_cut_image: function(){
		...
		var url = 'servtools/img_cut/cut_image.php';
		var pars = 'x=' + tmpPosition[0] + '&y=' + tmpPosition[1] + '&src=' + $('cut_image_path').value;
		pars = pars + '&w=' + $(this.image_id).offsetWidth + '&h=' +$(this.image_id).offsetHeight;
		
		var showResponse = function(originalRequest){
       $('msg').innerHTML = originalRequest.responseText;
       $('ico_action').style.display='';
		};
