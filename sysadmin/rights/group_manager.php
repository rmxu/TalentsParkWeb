...
function update_group(id_value){
	...
	if(group_value==''){
		...
		update_group.getInfo("group_manager.php?update=1","post","app","id="+id_value+"&value="+group_value,function(c){document.getElementById("group_"+id_value).innerHTML=group_value;change_add("show",id_value);}); 
	}
}
function check_form(){
...
<div id="maincontent">
    <div class="wrap">
        <div class="crumbs"><?php echo $ad_langpackage->ad_location;?> &gt;&gt; <a href="javascript:void(0);"><?php echo $ad_langpackage->ad_user_management;?></a> &gt;&gt; <a href="group_manager.php"><?php echo $ad_langpackage->ad_users_manage;?></a></div>
        <hr />
<div class="infobox">
			...
			<tr>
				...
				<td>
					<span id='show1_<?php echo $element['gid'];?>'>
						...
						<a href='group.php?gid=$id'><?php echo $ri_langpackage->ri_allot;?></a>&nbsp|&nbsp
						<a href='?del=del&id=<?php echo $element['gid'];?>'><?php echo $m_langpackage->m_del;?></a>
					</span>
					<span id='show2_<?php echo $element['gid'];?>' style='display:none'><?php echo $ri_langpackage->ri_user_name;?>：
						<input autocomplete='off' type='text' class='small-text' id='value_<?php echo $element['gid'];?>' value='<?php echo $element['name'];?>'/>&nbsp;
		...
		<tr>
			<td colspan=2>
				...
				<span id='add2_id' style='display:none'>
					<?php echo $ri_langpackage->ri_user_id;?>：<input autocomplete='off' type="text" class="small-text" id='add_id' name="id" value=""/> &nbsp 
					<?php echo $ri_langpackage->ri_user_name;?>：<input autocomplete='off' type="text" class="small-text" id='add_value' name="value" value=""/>&nbsp;
